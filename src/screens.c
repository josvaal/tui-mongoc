...
screen_id_t screen_connection(app_state_t *state) {
    ...
    mvwprintw(win, 9, 4, "mongodb://user:pass@localhost:27017/db?authSource=admin");
    ...
    wmove(win, 3, 2);
    bool got_input = input_get_string(win, 3, 2, state->uri_buffer,
                                      sizeof(state->uri_buffer), state->uri_buffer);
...
screen_id_t screen_database_list(app_state_t *state) {
    ...
    tui_draw_status(win, "UP/DOWN: Navigate | ENTER: Select | Q: Disconnect | F1: Help");
    ...
    while (true) {
        ...
        if (redraw) {
            ...
            // Draw databases
            for (int i = scroll_offset; i < state->db_count && i < scroll_offset + visible_lines; i++) {
...
screen_id_t screen_collection_list(app_state_t *state) {
    ...
    state->collections = mongo_list_collections(state->mongo_ctx, state->current_db,
                                                &state->coll_count);
    if (!state->collections) {
    ...
    if (state->coll_count == 0) {
        app_set_message(state, "No collections found in this database", MSG_WARNING);
    }
    ...
    char title[128];
    snprintf(title, sizeof(title), "Database: %s - Select Collection", state->current_db);
    ...
    tui_draw_status(win, "UP/DOWN: Navigate | ENTER: Select | C: Create | D: Delete | B: Back | Q: Disconnect | F1: Help");
    ...
    while (true) {
        ...
        if (redraw) {
            ...
            // Draw collections
            for (int i = scroll_offset; i < state->coll_count && i < scroll_offset + visible_lines; i++) {
                int y = 3 + (i - scroll_offset);
        ...
        if (IS_KEY_UP(ch) && selected > 0) {
        ...
        else if ((ch == '\n' || ch == KEY_ENTER || ch == 10 || ch == 13) && state->coll_count > 0) {
            ...
            if (input_text_single("Create Collection", "Collection name:", coll_name, sizeof(coll_name),
                                 "Enter the name for the new collection")) {
                if (!is_empty_string(coll_name)) {
                    if (mongo_create_collection(state->mongo_ctx, state->current_db, coll_name)) {
                        app_set_message(state, "Collection created successfully!", MSG_SUCCESS);
                        delwin(win);
                        return SCREEN_COLLECTION_LIST;
                    } else {
                        char err_msg[256];
                        snprintf(err_msg, sizeof(err_msg), "Failed to create collection: %s",
                                mongo_get_error(state->mongo_ctx));
            ...
            if (tui_confirm("Delete Collection", confirm_msg)) {
                if (mongo_drop_collection(state->mongo_ctx, state->current_db,
                                         state->collections[selected])) {
                    app_set_message(state, "Collection deleted successfully!", MSG_SUCCESS);
                    delwin(win);
                    return SCREEN_COLLECTION_LIST;
                } else {
                    char err_msg[256];
...
bool load_documents(app_state_t *state) {
    ...
    // Get total count
    state->total_documents = mongo_count_documents(state->mongo_ctx, state->current_db,
    ...
    state->documents = mongo_find_documents(state->mongo_ctx, state->current_db,
                                           state->current_collection, state->current_filter,
                                           skip, state->doc_per_page, &state->doc_count);
...
screen_id_t screen_document_viewer(app_state_t *state) {
    ...
    char title[256];
    snprintf(title, sizeof(title), "%s.%s - Documents", state->current_db, state->current_collection);
    tui_draw_box(win, title);

    int total_pages = (state->total_documents + state->doc_per_page - 1) / state->doc_per_page;
    if (total_pages == 0) total_pages = 1;

    tui_draw_status(win, "UP/DOWN: Select | PgUp/PgDn: Page | I: Insert | E: Edit | D: Delete | B: Back | R: Refresh");
    ...
    while (true) {
        if (redraw) {
            ...
            for (int i = 0; i < state->doc_count && y < LINES - 4; i++) {
                ...
                if (i == state->doc_selected) {
                    wattron(win, COLOR_PAIR(COLOR_PAIR_SELECTED));
                    snprintf(doc_header, sizeof(doc_header), " > Document %d: [SELECTED]", doc_num);
        ...
        if (IS_KEY_UP(ch) && state->doc_selected > 0) {
            ...
            // Show editor
            bool edited = input_text_editor("Edit Document", json_buffer, sizeof(json_buffer),
            ...
            if (edited && !is_empty_string(json_buffer)) {
                ...
                bson_iter_t iter;
                if (bson_iter_init_find(&iter, state->documents[state->doc_selected], "_id")) {
                ...
                // Perform update
                long long modified = mongo_update_documents(state->mongo_ctx, state->current_db,
                                                           state->current_collection, filter,
                                                           update, false);
            ...
            if (tui_confirm("Delete Document", "Are you sure you want to delete this document?")) {
                ...
                long long deleted = mongo_delete_documents(state->mongo_ctx, state->current_db,
                ...
                if (deleted > 0) {
                    ...
                    if (state->doc_selected >= state->doc_count - 1) {
                        ...
                        if (state->doc_selected < 0) state->doc_selected = 0;
                    }
                    delwin(win);
                    return SCREEN_DOCUMENT_VIEWER;
                } else {
...
screen_id_t screen_document_insert(app_state_t *state) {
    ...
    bool result = input_text_editor("Insert Document", json_buffer, sizeof(json_buffer),
